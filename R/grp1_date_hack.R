...
xgb.DMatrix.save(dtrain, "./input/dtrain.data")
xgb.DMatrix.save(dmodel, "./input/dmodel.data")
xgb.DMatrix.save(dvalid, "./input/dvalid.data")
gc(verbose=FALSE)
...
gc(verbose=FALSE)
cat("Exporting SVMLight format\n")
xgb.DMatrix.save(dtest, "./input/dtest.data")
gc(verbose=FALSE)
...
submit2 <- fread("./metafeatures/prfull_xgb_hash_bag_20160827_dataxgb_hash_bag.csv")
...
submit4$filled.x[is.na(submit4$filled.x)] <- submit4$xgb_hash_bag0[is.na(submit4$filled.x)]
submit5 <- data.frame(activity_id = submit4$activity_id, outcome = submit4$filled.x, stringsAsFactors = FALSE)
write.csv(submit5, file="./submissions/xgb10bag-filled.csv", row.names=FALSE)
